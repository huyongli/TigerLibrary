package cn.ittiger.library.ui.recycler;
...
public abstract class CommonRecyclerViewAdapter<T> extends RecyclerView.Adapter<CommonRecyclerViewAdapter.CommonViewHolder> {
    ...
    /**
     * 当前position是否在数据索引范围内
     * @param position
     * @return
     */
    public boolean isValidDataPosition(int position) {

        return position >= getHeaderViewCount() && position < getHeaderViewCount() + getItemDataCount();
    }
     ...
     * @return
     */
    public int getLastValidDataPosition() {

        return getHeaderViewCount() + getItemDataCount() - 1;
    }
     ...
     * @return
     */
    public int getFirstValidDataPosition() {
    ...
    public void removeFooterView(View footerView) {
        ...
        for(int i = 0; i < mFooterViews.size(); i++) {
            int key = mFooterViews.keyAt(i);
            if(footerView == mFooterViews.get(key)) {
                mFooterViews.remove(key);
                notifyItemRangeRemoved(getHeaderViewCount() + getItemDataCount() + i, 1);
                break;
            }
        }
    }
